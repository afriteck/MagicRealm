...
public class HexTiles extends JPanel {
	 ...
	 * for(int i=0; i<20; i++){
	 * if(cells[i].tilehex.getName().equals(tilename)){cells
	 * [i].setPlayerImg(tileclearing);} } }
	 */
	...
	private void initHexCells(double w, double h, double R, Rectangle range) {
		...
		/*
		 * JSONParser parser = new JSONParser(); long xPos, yPos;
		 * ArrayList<Clearing> clearingList = null; Clearing currClearing; Tiles
		 * tile = null;
		 * 
		 * try { JSONArray a = (JSONArray) parser.parse(new
		 * FileReader("/Z:/eclipse-Juno-Workspace/MagicRealm/bin/res/data.json"
		 * ));
		 * 
		 * for(Object o : a) { JSONObject currTile = (JSONObject) o;
		 ...
		 * 
		 * for(int i = 0; i <=6; i++) {
		 * if(clearings.containsKey(Integer.toString(i))) { JSONObject
		 * clearingNum = (JSONObject) clearings.get(Integer.toString(i)); xPos =
		 * (long) clearingNum.get("x"); yPos = (long) clearingNum.get("y");
		 * 
		 * currClearing = new Clearing(i, xPos, yPos);
		 * clearingList.add(currClearing); } } tile = new Tiles(tileName,
		 * tileFileName, clearingList); bt.addTile(tile); } } catch (Exception
		 * e) { e.printStackTrace(); }
		 */
		...
		for (int i = 0; i < list.size(); i++) {
			...
			// img =
			// Toolkit.getDefaultToolkit().getImage(HexTiles.class.getResource("/"
			// + bt.getFileName(i)));
		...
		/*
		 * //cells[0].setImg(Toolkit.getDefaultToolkit().getImage(HexTiles.class.
		 * getResource("/" + cells[0].tilehex.getFilePath())));
		 * cells[0].setTile(bt.getTile("OAK WOODS"));
		 * cells[0].tilehex.setTheta(60); //System.out.println("/" +
		 * cells[0].tilehex.getName()); try { URL url =
		 * BoardTest.class.getResource("/" + cells[0].tilehex.getFilePath());
		 * cells[0].setImg(ImageIO.read(new File(url.getPath()))); } catch
		 * (IOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
		 * 
		 * cells[2].setTile(bt.getTile("CAVES"));
		 * cells[2].tilehex.setTheta(180); //System.out.println("/" +
		 * cells[0].tilehex.getName()); try { URL url =
		 * BoardTest.class.getResource("/" + cells[2].tilehex.getFilePath());
		 * cells[2].setImg(ImageIO.read(new File(url.getPath()))); } catch
		 * (IOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
		 */
	}
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (id.equals("Show Index")) {
					...
					repaint();
						 ...
						 * if(id.equals("tip high")) { tipHigh = selected; }
						 */
	...
	/*
	 * public static void main(String[] args) { if(args.length > 0) debug =
	 * args[0].equals("-d"); HexTiles test = new
	 * HexTiles(BoardTest.initializeTiles()); JFrame f = new JFrame();
	 * f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); f.add(test);
	 * f.add(test.getControls(), "Last"); f.pack(); f.setLocation(100,100);
	 * f.setVisible(true); test.addMouseListener(test.switcher);
...
/*
 * class HexCell { String id; Point2D.Double center; Shape shape; boolean
 * showIndex; double start; String[] neighbors; boolean isSelected = false;
 * BufferedImage tileimg; BufferedImage rotated; Tiles tilehex;
 * //List<Point2D.Double> clearingList = new ArrayList<Point2D.Double>();
 * 
 * 
 * 
 * public HexCell(String id, Point2D.Double center, Shape shape, boolean
 * showCenter, double start, String[] neighbors, Tiles tile) { this.id = id;
 * this.center = center; this.shape = shape; this.showIndex = showCenter;
 * this.start = start; this.neighbors = neighbors; this.tilehex = tile; try {
 * this.tileimg = ImageIO.read(new
 * File("/Z:/eclipse-Juno-Workspace/MagicRealm/src/" + tilehex.getFilePath()));
 * } catch (IOException e) { // TODO Auto-generated catch block
 * e.printStackTrace(); } //rotateTile(); }
 * 
 * public HexCell(String id, Point2D.Double center, Shape shape, boolean
 * showCenter, double start, String[] neighbors) { this.id = id; this.center =
 * center; this.shape = shape; this.showIndex = showCenter; this.start = start;
 * this.neighbors = neighbors; }
 * 
 * public void setImg(BufferedImage img){ tileimg = img; //rotateTile(); }
 * 
 * public void setTile(Tiles tile){ tilehex = tile; }
 * 
 * public void rotateTile() {
 * 
 * if(tilehex != null && tileimg != null){ double angle = tilehex.getTheta();
 * 
 * int w = tileimg.getWidth(); int h = tileimg.getHeight();
 * 
 * rotated = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB); Graphics2D
 * g2d = rotated.createGraphics(); AffineTransform at = new AffineTransform();
 * at.translate((w / 2), (h / 2));
 * 
 * at.rotate(Math.toRadians(angle)); at.translate((-w / 2), (-h / 2));
 * g2d.setTransform(at);
 * 
 * g2d.drawImage(tileimg, 0, 0, null);
 * 
 * tileimg = rotated; System.out.println("rotated " + angle); g2d.dispose(); } }
 * 
 * public void draw(Graphics2D g2) { if(showIndex) { g2.setPaint(Color.red);
 * g2.fill(new Ellipse2D.Double(center.x-1.5, center.y-1.5, 4, 4));
 * g2.drawString(id, (float)(center.x+3), (float)(center.y+3));
 * g2.setPaint(Color.black); } if(tilehex != null){g2.draw(shape); /*
 * BufferedImage scaled = new BufferedImage(tileimg.getWidth() / 2,
 * tileimg.getHeight() / 2, BufferedImage.TYPE_INT_ARGB); Graphics2D g2d =
 * scaled.createGraphics();
 * g2d.setTransform(AffineTransform.getScaleInstance(0.5d, 0.5d));
 * g2d.drawImage(tileimg, 0, 0, null); g2d.dispose(); tileimg = scaled;
 * setAngle(0d);
 */
...
/*
 * if(tilehex != null){ switch (tilehex.getTheta()) { case 0:
 * g2.rotate(Math.toRadians(0)); case 60: g2.rotate(Math.toRadians(60)); case
 * 120: g2.rotate(Math.toRadians(120)); case 180:
 * g2.rotate(Math.toRadians(180)); case 240: g2.rotate(Math.toRadians(240));
 * case 300: g2.rotate(Math.toRadians(300)); default: } } //rotateTile();
 * if(tilehex != null && tileimg != null){ double angle = tilehex.getTheta();
 * 
 * int w = tileimg.getWidth(); int h = tileimg.getHeight();
 * 
 ...
 * g2d.setTransform(at);
 * 
 * g2d.drawImage(tileimg, 0, 0, null); Shape cl = null; BufferedImage chitimg =
 * null; for(int i=0; i<tilehex.getClearings().size(); i++){ //g2d.draw(new
 * Ellipse2D.Double((tilehex.getClearings().get(i).getXposition()),
 * (tilehex.getClearings().get(i).getYposition()), 80, 80)); //chitimg =
 * ImageIO.read(new File("/Z:/eclipse-Juno-Workspace/MagicRealm/src/" +
 * tilehex.getClearings().get(i).getSoundChits().get(i).getFilePath())); cl =
 * at.createTransformedShape(new
 * Ellipse2D.Double((tilehex.getClearings().get(i).getXposition() - 40),
 * (tilehex.getClearings().get(i).getYposition() - 40), 80, 80));
 * g2d.drawImage(chitimg, tilehex.getClearings().get(i).getXposition() - 40,
 * tilehex.getClearings().get(i).getYposition() - 40, cl.getBounds().width,
 ...
 * shape.getBounds().width, shape.getBounds().height, null); //g2.draw(c1);
 * //tileimg = rotated; System.out.println("rotated " + angle); g2d.dispose(); }
 * //g2.drawImage(tileimg, shape.getBounds().x, shape.getBounds().y,
 * shape.getBounds().width, shape.getBounds().height, null); //g2.draw(new
 * Ellipse2D.Double(center.x-1.5, center.y-1.5, 30, 30)); }
 * 
 * 
 * //g2.setTransform(saveTransform);
 * 
 * if(isSelected) { System.out.println(tilehex); toggleSelection(); } }
 * 
 * public void setShowIndex(boolean show) { showIndex = show; }
 * 
 * public void toggleSelection() { isSelected = !isSelected; }
 * 
 * public boolean contains(Point p) { return shape.contains(p); }
 * 
 * public String toString() { return "HexCell[id:" + id + ", neighbors:" +
