...
public class Gui extends JFrame implements MouseListener {
	...
	private JButton rollButton, selectButton, sbackButton, backButton, move,
			hide, search, trade, rest, alert, hire, follow, enchant;
	...
	private CharacterContainer characters = new CharacterContainer(); // bag of
																		// things

	private Player currentPlayer = gm.getPlayer1(); // the turn of thGDGFGFe
													// current player.

	private Things selectedCharacter = null; // when the player clicks a
												// Character off their rack

	private LinkedList<Things> p1characters = new LinkedList<Things>(),
			p2characters = new LinkedList<Things>(),
			p3characters = new LinkedList<Things>(),
			p4characters = new LinkedList<Things>();
	...
	private JLabel amazon, blackKnight, captain, dwarf, elf, swordsman,
			AmazonIcon, CaptainIcon, BlackKnightIcon, SwordsManIcon, DwarfIcon,
			ElfIcon;
	...
	public Gui() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				Gui.class.getResource("/others/rs_logo.jpg")));
		setVisible(true);
		...
		p1button.setSelectedIcon(new ImageIcon(Gui.class
				.getResource("/others/p1s.png")));
		...
		p2button.setSelectedIcon(new ImageIcon(Gui.class
				.getResource("/others/p2s.png")));
		...
		move.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				System.out.println(currentPlayer.getName()
						+ currentPlayer.getTile() + currentPlayer.getClearing());
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (gm.requestHire(currentPlayer, txt)) {
					...
					for (int i = 0; i < ng.size(); i++)
						gm.showNativeGroup(ng.get(i), currentPlayer, txt,
								getContentPane());
					if (turn >= 5) {
						getNextPlayer(currentPlayer);
						turn = 1;
		...
		RollButton.addActionListener(new ActionListener() { // Button Listener/*
					...
					public void actionPerformed(ActionEvent e) {
						...
						// setPlayer1Roll
						// (currentPlayer.getPlayerDie().getDieRoll(currentPlayer.getDieBool(),
						// getLastRoll(), getLastRoll2()));
						...
						if (currentPlayer.getCharacter() != null) {
							currentPlayer.getCharacter().setRoll(
									Math.max(lastRoll, lastRoll2));

							txt.append("\n----------------------------------\n"
									+ currentPlayer.getName() + " rolled : "
									+ currentPlayer.getCharacter().getRoll());
						}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (cheatMode) {
					...
					boardt.getTile("AWFUL VALLEY").getClearingByNum(5)
							.setDwelling(chapel);
					boardt.getTile("BAD VALLEY").getClearingByNum(5)
							.setDwelling(guardhouse);
					boardt.getTile("CURST VALLEY").getClearingByNum(5)
							.setDwelling(house);
					boardt.getTile("DARK VALLEY").getClearingByNum(5)
							.setDwelling(inn);
		...
		amazon.setIcon(new ImageIcon(Gui.class
		...
		dwarf.setIcon(new ImageIcon(Gui.class
		...
		captain.setIcon(new ImageIcon(Gui.class
		...
		blackKnight.setIcon(new ImageIcon(Gui.class
		...
		swordsman.setIcon(new ImageIcon(Gui.class
		...
		for (int i = 0; i < 6; i++)
			characterLabels.get(i).addMouseListener(this);
		...
		AmazonIcon.setIcon(new ImageIcon(Gui.class
		...
		CaptainIcon.setIcon(new ImageIcon(Gui.class
		...
		DwarfIcon.setIcon(new ImageIcon(Gui.class
		...
		ElfIcon.setIcon(new ImageIcon(Gui.class
		...
		SwordsManIcon.setIcon(new ImageIcon(Gui.class
		...
		BlackKnightIcon.setIcon(new ImageIcon(Gui.class
		...
		sbackButton.setIcon(new ImageIcon(Gui.class
				.getResource("/actions/backarrow.gif")));
		sbackButton.setVisible(false);
	 ...
	 * 
	 * 
	 * if (p == player1) { if (t == 1) { p1character = new Things();
	 ...
	 * }
	 * 
	 * if (p == player2){ if (t == 1) { p2character = new Things();
	 * p2character.setName(((Component) e.getSource()).getName());
	 * p1characters.add(p2character); donePlayerRound = true; }
	 * 
	 * } txt.append("\n"); characterSelectionPhase = false;
	 * getNextPlayer(currentPlayer); if (!donePlayerRound)
	 * characterSelectionPhase = true; else { characterSelectionPhase = false;
	 * txt.append(
	 * "\n---------------------------------\nPlayer characters have been chosen.\n"
	 * ); donePlayerRound = false; startGame = true;
	 * 
	 * } }
	 */
	...
	public void getRecruitThing(MouseEvent e) {
		...
		for (int i = 0; i < characters.getCharacterContainerSize(); i++) {
			if (((Component) e.getSource()).getName() == characters
					.getCharacterContainer().get(i).getName()) {
	...
	public void showMessage(Things ths, Player p) {
		txt.append(p.getName() + " Chose " + ths.getName() + "\n"
				...
				+ ths.getWeapons().getName() + " has a length of "
				+ ths.getWeapons().getLength()
	...
	public void checkCharacter(Player p, MouseEvent e) { // check the character
		...
		if (p == gm.getPlayer2()) {
			selected = true; // we now know that p2 is done selecting so,
								// selecion phase is over
			if (name == "Amazon") {
				Amazon amazon = new Amazon();
				gm.getPlayer2().setCharacter(amazon);
				// player2.setCharacter(new Amazon());

				txt.append(gm.getPlayer2().getName() + " Chose "
						+ gm.getPlayer2().getCharacter().getName() + "\n"
						+ gm.getPlayer2().getCharacter().getName()
						+ " currently visibile? "
						+ gm.getPlayer2().getCharacter().getVisibility() + "\n");
	...
	public void toggleDie1() { // change the die1 face any time the die is
								// clicked
	...
	/*
	 * public void moveActivity(Player p, ActionEvent e){ if(!moveAct){ int
	 * dialogButton = JOptionPane.YES_NO_OPTION; int dialogResult =
	 * JOptionPane.showConfirmDialog (null,
	 * "ARE YOU SURE YOU WANT TO DO THIS ACTIVITY FOR THE DAY?"
	 * ,"Alert",dialogButton); if (dialogResult == JOptionPane.YES_OPTION){
	 * moveAct = true;
	 * 
	 * txt.append(
	 * "\n---------------------------------- WHERE WOULD YOU LIKE TO MOVE!!\n");
	 * } else{ moveAct = false; JOptionPane.showMessageDialog(getContentPane(),
	 * "PLEASE CHOOSE YOUR ACTIVITIES FOR THE DAY.");
	 * 
	 * } } else if (activityCounter == 0){
	 * JOptionPane.showMessageDialog(getContentPane(),
	 * "YOU HAVE USED ALL YOUR REQUIRED ACTIVITIES FOR THE DAY."); moveAct =
	 * false; } //getNextPlayer(currentPlayer); //movePlayer(currentPlayer, e);
	 * 
	 * if(moveAct & p == player2 & activityCounter == 0){ moveAct = false;
	 * doneMove = true; txt.append("done activity choosing");
	 * JOptionPane.showMessageDialog(getContentPane(), "we are done here.");
	 * move.setVisible(false); hide.setVisible(false); search.setVisible(false);
	 * rest.setVisible(false); trade.setVisible(false); } else{ doneMove =
	 * false; }
	 * 
	 * }
	 * 
	 * public void movePlayer(Player p, ActionEvent e){ moveActivity(p, e);
	 * 
	 * 
	 * }
	 */
	...
	public void searchAction() {
		...
		// locate.setSelectedIcon(new
		// ImageIcon(Gui.class.getResource("/others/p1s.png")));
	...
	public void checkSelected(Player p, MouseEvent e) {
		...
		if (selected & p == gm.getPlayer2()) {
			...
			if (!cheatMode && gm.isSelected()) {
				...
				boardt.getTile("BAD VALLEY").getClearingByNum(5)
						.movePersonHere(gm.getPlayer1().getPchit());
				boardt.getTile("BAD VALLEY").getClearingByNum(5)
						.movePersonHere(gm.getPlayer2().getPchit());
