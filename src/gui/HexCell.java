		...
		if (tilehex != null) {
			...
			if (tilehex != null && tileimg != null) {
				...
				for (int i = 0; i < tilehex.getClearings().size(); i++) {
					 ...
					 * System.out.println("Placed player image"); URL url =
					 * InitBoardTiles.class.getResource("/monsters/dragon.gif");
					 * try { chitimg = ImageIO.read(new File(url.getPath()));
					 * saveAt.setTransform(at); at.translate((w / 2), (h / 2));
					 * at.rotate(Math.toRadians(-angle)); at.translate((-w / 2),
					 * (-h / 2)); g2d.setTransform(at); g2d.drawImage(chitimg,
					 * cl.getBounds().x, cl.getBounds().y, cl.getBounds().width,
					 * cl.getBounds().height, null); at.setTransform(saveAt); }
				...
				// g2d.draw(new Ellipse2D.Double(center.x-1.5, center.y-1.5, 30,
				// 30));
			...
			// shape.getBounds().width, shape.getBounds().height, null);
			// g2.draw(new Ellipse2D.Double(center.x-1.5, center.y-1.5, 30,
			// 30));
		}
