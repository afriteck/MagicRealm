...
public abstract class Things implements Move, Business, Search, Hide {
	...
	private LinkedList<Armor> myArsenal; // the characters arsenal
	...
	/*
	 * public PlayerChit getPchit() { return pchit; }
	 * 
	 * public void setPchit(PlayerChit pchit) { this.pchit = pchit; }
	 */
	...
	public void addNative(LinkedList<NativeGroup> hiredN,
	...
	public void sellArmor(Things th, Armor arm) {
			...
			if (th.getCharacterArmory().get(i).getName() == arm.getName()) {
				...
				if (arm.isDamaged()) {
					...
					JOptionPane.showMessageDialog(null,
							"You have sold your " + arm.getName()
									+ " your current gold is " + th.getGold()
									+ " golds");
		...
		JOptionPane.showMessageDialog(null,
				"You have do not have the specified armor to sell");
	...
	public void sellWeapon(Things th, Weapons arms) {
		...
		if (th.getMyweapons().size() == 1
				&& th.getWeapons().getName() == arms.getName()) {
			th.getMyweapons().remove(arms);
			th.setWeapon(null);
			JOptionPane.showMessageDialog(null,
					"You have sold your one and only weapon!");
	...
	public void buyNative(Things th, NativeGroup nt) { // you can recruit a
		...
		if (th.getGold() >= nt.getCost()) {
			...
			JOptionPane.showMessageDialog(null,
					"You have successfully acquired " + nt.getName()
							+ " Native Group");
			th.setGold(th.getGold() - nt.getCost());
		} else {
			JOptionPane
					.showMessageDialog(null, "You cannot afford this native");
	...
	/*
	 * public void buyNative(Things th, LinkedList<NativeGroup> ng, NativeGroup
	 * nt) {
	 * 
	 * if(th.getGold() >= nt.getCost()){
	 * 
	 * this.hiredNatives = ng; this.hiredNative = nt; th.setHiredNatives(ng);
	 * 
	 * ng.add(nt);
	 * 
	 * th.setGold(th.getGold()- nt.getCost());
	 * JOptionPane.showMessageDialog(null, "You have successfully acquired " +
	 * nt.getName() + " Native Group");
	 * 
	 * }else{
	 * 
	 * System.out.println("You do not have enough gold to hire this native");
	 * JOptionPane.showMessageDialog(null,
	 * "You do not have enough gold to buy this native group ");
	 * 
	 * 
	 * }
	 * 
	...
	public void buyArmor(Things th, Armor arm) {
		if (arm.isDamaged() && th.getGold() >= arm.getDamagedPrice()) {
			...
			th.addArmor(arm);
			JOptionPane.showMessageDialog(null,
					"You have purchased " + arm.getName()
			...
			JOptionPane.showMessageDialog(null,
					"You cannot afford the selected armor");
