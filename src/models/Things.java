...
public abstract class Things implements Move, Business, Search, Hide{
    ...
    private ArrayList<Armor> myArsenal;    // the chatacters arsenal
    private ArrayList<Things> friends;
    private ArrayList<Things> enemies;
...
public ArrayList<Armor> getCharacterArmory() {
    return myArsenal;
}

public ArrayList<NativeGroup> getHiredNatives() {
	//hiredNatives = new LinkedList<NativeGroup>();
	return hiredNatives;
}

public void setHiredNatives(ArrayList<NativeGroup> hiredNatives) {
...
public ArrayList<Things> getFriends() {
    return friends;
}

public ArrayList<Things> getEnemies() {
    return enemies;
}

public void setFriends(ArrayList<Things> characterFriends) {
    this.friends = characterFriends;
}

public void setEnemies(ArrayList<Things> characterEnemies) {
    this.enemies = characterEnemies;
}

public void setPlayerArmoury(ArrayList<Armor> characterArsenal) {
...
public void addNative(ArrayList<NativeGroup> hiredN, NativeGroup aNativeGroup) {
	
	hiredN = new ArrayList<NativeGroup>();
...
public ArrayList<Weapons> getMyweapons() {
	return myweapons;
}

public void setMyweapons(ArrayList<Weapons> myweapons) {
...
public void useCounter(Counters usedcounter){
	
	fatiguedCounter.add(usedcounter);
	if(readyCounter.size() > 0){
		for(int i = 0; i < readyCounter.size(); i++)
			readyCounter.remove(usedcounter);
	}
}

public void rest(Counters counter){
	
	if (fatiguedCounter.size() > 0){
		fatiguedCounter.remove(counter);
		readyCounter.add(counter);
