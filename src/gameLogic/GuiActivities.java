...
public class GuiActivities {
	...
	public boolean requestMove(Player p) {
		...
		if (option == JOptionPane.OK_OPTION) {
			...
			if (bd.getTile(p.getCharacter().getTileName()).isNeighbour(
					bd.getTile(input)) == true
				...
				bd.getTile(p.getTile()).getClearingByNum(p.getClearing())
	...
	public boolean placePlayer1(Player p) {
		int option = JOptionPane.showConfirmDialog(null, message,
				"Place Player 1!", JOptionPane.OK_CANCEL_OPTION);
	...
	public void showNativeGroup(final NativeGroup ng, final Player p,
			final JTextArea txt, Container cp) {
	...
	public void cheatmodeButtons(Container container) {
		...
		chapelLabel.setIcon(new ImageIcon(Gui.class
				.getResource("/dwellings_c/chapel.gif")));
		...
		guardLabel.setIcon(new ImageIcon(Gui.class
				.getResource("/dwellings_c/guard.gif")));
		...
		houseLabel.setIcon(new ImageIcon(Gui.class
				.getResource("/dwellings_c/house.gif")));
		...
		innLabel.setIcon(new ImageIcon(Gui.class
				.getResource("/dwellings_c/inn.gif")));
	...
	public void toggleDie1(int clicked, int lastRoll, JButton die1,
			ImageIcon[] img) { // change the die1 face any time the die is
								// clicked
		...
		if (clicked == 1) {
			...
			;
			clicked = 1;
			toggleDie1(clicked, lastRoll, die1, img);
		}

	}

	public void toggleDie2(int clicked, int lastRoll2, JButton die2,
			ImageIcon[] img) {
		...
		if (clicked == 1) {
			...
			;
			clicked = 1;
			toggleDie2(clicked, lastRoll2, die2, img);
		}
